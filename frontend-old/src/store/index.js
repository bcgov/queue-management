...
export const store = new Vuex.Store({
  ...
  getters: {
    ...
    on_hold_queue(state) {
      ...
      let isCitizenOnHold = function(c) {
        let test = c.service_reqs.filter(sr =>
    ...
    categories_options: (state, getters) => {
      ...
      let opts = state.categories.filter(o =>
  ...
  actions: {
    ...
    getServices(context) {
      ...
      Axios(context)
        ...
        .then(resp => {
          let services = resp.data.services.filter(
      ...
      if (!(context.state.serviceBegun && context.state.citizenInvited)) {
        ...
        citizens.forEach(citizen => {
          if (citizen.service_reqs.length > 0) {
            let activeService = citizen.service_reqs.filter(sr =>
              sr.periods.some(p => p.time_end === null)
            );
    ...
    messageFeedback(context) {
      ...
      if (activeCitizen) {
        let activeService = activeCitizen.service_reqs.filter(sr =>
          sr.periods.some(p => p.time_end === null)
        )[0];
        let activePeriod = activeService.periods.filter(
          p => p.time_end === null
        )[0];
    ...
    postITAChallengerExam(context) {
      ...
      let exam_type = context.state.examTypes.find(
    ...
    postITAIndividualExam(context, isRequestExam) {
      ...
      defaultValues.number_of_students =
        responses["ind_or_group"] === "group"
          ...
          : 1;
      let exp = new moment(responses.expiry_date)
        .format("YYYY-MM-DD")
        .toString();
      ...
      if (responses["ind_or_group"] === "group") {
        let examType = context.state.examTypes.find(
          type =>
  ...
  mutations: {
    ...
    setDefaultChannel(state) {
      let channel = state.channels.filter(
...
let makeBookingReqObj = (context, responses) => {
  ...
  let length = context.state.examTypes.find(
