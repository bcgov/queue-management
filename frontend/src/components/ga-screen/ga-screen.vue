      ...
      this.sortedCsrs.forEach(csr => {
        ...
        if (activeCitizen === null) {
          ...
          if (activeCitizen.service_reqs[0].periods.filter(p => p.ps.ps_name === 'Being Served')[0]) {
            ...
            const waitDate = new Date(null)
            if (waitPeriods.length !== 0) {
              const waitStart = new Date(waitPeriods[0].time_start)
              const waitEnd = new Date(waitPeriods[0].time_end)
            ...
            const citizenStartDate = new Date(activeCitizen.start_time)
            const firstServedPeriodDate = new Date(firstServedPeriod.time_start)
            let timeServeClosed = 0
            let timeServeOpen = timeServeClosed
            activeServiceRequest.periods.forEach(p => {
              if (p.ps.ps_name === 'Being Served') {
                if (p.time_end != null) {
                  const dateEnd = new Date(p.time_end)
                  const dateStart = new Date(p.time_start)
            ...
            const waitSeconds = (firstServedPeriodDate - citizenStartDate) / 1000
