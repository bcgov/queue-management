                ...
                for i in each['service_reqs']:
                    ...
                    if served_period:
                        if served_period['ps']['ps_name'] == 'Being Served':
                            ...
                            print(datetime.utcnow(),'==', served_period['time_start'], '====', datetime.utcnow()-datetime.strptime(served_period['time_start'], '%Y-%m-%dT%H:%M:%S.%f'))
                            print(data_dict['service_begin_seconds'], type(datetime.utcnow()-datetime.strptime(served_period['time_start'], '%Y-%m-%dT%H:%M:%S.%f')), (datetime.utcnow()-datetime.strptime(served_period['time_start'], '%Y-%m-%dT%H:%M:%S.%f')).seconds)
                            
                            # data_dict['service_begin_seconds'] = (datetime.utcnow()-datetime.strptime(served_period['time_start'], '%Y-%m-%dT%H:%M:%S.%f')).total_seconds()
                            data_dict['service_begin_seconds'] = (datetime.utcnow()-datetime.strptime(served_period['time_start'], '%Y-%m-%dT%H:%M:%S.%f')).secondsserving_app.append(data_dict)
                            data_dict = {}
                            break
