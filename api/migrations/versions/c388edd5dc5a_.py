...
def upgrade():
    ...
    with op.batch_alter_table('appointment_version', schema=None) as batch_op:
        batch_op.drop_index(batch_op.f('ix_appointment_version_end_transaction_id'))
        batch_op.drop_index(batch_op.f('ix_appointment_version_operation_type'))
    ...
    with op.batch_alter_table('exam', schema=None) as batch_op:
        ...
        batch_op.alter_column('exam_received_date',
               existing_type=postgresql.TIMESTAMP(timezone=True),
               type_=sa.DateTime(),
               existing_nullable=True)
        batch_op.alter_column('exam_returned_date',
               existing_type=postgresql.TIMESTAMP(timezone=True),
               type_=sa.DateTime(),
               existing_nullable=True)
    ...
    with op.batch_alter_table('service', schema=None) as batch_op:
        batch_op.alter_column('external_service_name',
               existing_type=sa.VARCHAR(length=500),
               type_=sa.String(length=100),
               existing_nullable=True)
        batch_op.alter_column('online_link',
               existing_type=sa.VARCHAR(length=500),
...
def downgrade():
    ...
    sa.Column('citizen_name', sa.VARCHAR(length=255), autoincrement=False, nullable=True),
    sa.Column('contact_information', sa.VARCHAR(length=255), autoincrement=False, nullable=True),
    sa.Column('blackout_flag', sa.VARCHAR(length=1), autoincrement=False, nullable=True),
    sa.Column('recurring_uuid', sa.VARCHAR(length=255), autoincrement=False, nullable=True),
    ...
    sa.PrimaryKeyConstraint('appointment_id', 'transaction_id', name=op.f('appointment_version_pkey'))
    )
    with op.batch_alter_table('appointment_version', schema=None) as batch_op:
        batch_op.create_index(batch_op.f('ix_appointment_version_transaction_id'), ['transaction_id'], unique=False)
        batch_op.create_index(batch_op.f('ix_appointment_version_operation_type'), ['operation_type'], unique=False)
        batch_op.create_index(batch_op.f('ix_appointment_version_end_transaction_id'), ['end_transaction_id'], unique=False)
