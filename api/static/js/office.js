...
$(document).ready(function() {
    ...
    online_status.on('change', function (e) {        
        if (e.val === "HIDE") { // if Hidden
            toggleOnlineStatusFields(false);
        } else { // if NOT Hidden
            if (appointments_enabled_ind.val() === "0") {
                toggleOnlineStatusFields(false);
...
function toggleOnlineStatusFields(flag) {
    if (flag) {
        ...
        $("label[for='latitude']").parent().show();
        $("label[for='longitude']").parent().show();
        ...
        $("label[for='timeslots']").parent().show();
    } else {
        $("label[for='office_appointment_message']").parent().hide();
        $("label[for='latitude']").parent().hide();
        $("label[for='longitude']").parent().hide();
        ...
        $("label[for='timeslots']").parent().hide();
    }
}