...
def migrate(message):
    ...
    fm.migrate(message=message)

@db_cli.command()
@click.argument('revision', default='head')
def upgrade(revision):
    '''Upgrade the database.'''
    fm.upgrade(revision=revision)

@db_cli.command()
@click.argument('revision', default='-1')
def downgrade(revision):
    '''Downgrade the database.'''
    fm.downgrade(revision=revision)
...
def bootstrap():
    '''Sets up the database with enough information to do development or run demos.'''
...
@application.cli.command('migrate_db')
def migrate_wrapper():
    '''Shortcut for "db upgrade".'''
